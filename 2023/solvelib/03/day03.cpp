  ...
  std::for_each(std::execution::par_unseq, rows.begin(), rows.end(),
                [ncols, this, &numbers](const std::size_t row) {
                  ...
                  for (std::size_t col = 0; col < ncols; ++col) {
                    ...
                    if (!isnum(input[idx])) {
                      if (col - num_begin != 0) {
                        xlog::debug("Found number {} in row {}, cols [{}, {})",
                                    num, row, num_begin, col);
                        std::fill(
                            numbers[row].begin() + std::ptrdiff_t(num_begin),
  ...
  return std::transform_reduce(
      ...
      [this, nrows, ncols, &numbers](const std::size_t idx) -> std::int64_t {
        ...
        if (auto sz = neighbours.size(); sz != 2) {
          xlog::debug("Gear in row {} has {} neighbour{}", row, sz,
                      sz > 1 ? "s" : "");
          return 0;
        }

        xlog::debug("Gear in row {} has neighbours {} and {}", row,
