...
struct hash_map {
  ...
  void insert(key_t&& k, value_t&& v) {
    ...
    auto it = std::ranges::find_if(bucket, [&k](auto const& pair) { return pair.first == k; });
    if (it == bucket.end()) {
      bucket.insert(it, std::make_pair(std::forward<key_t>(k), std::forward<value_t>(v)));
      return;
    }
...
std::uint64_t Day15::part1() {
  auto view = std::string_view(input.begin(), input.end() - 1) | std::ranges::views::split(',');

  return std::transform_reduce(std::execution::par_unseq, view.begin(), view.end(),
...
std::uint64_t Day15::part2() {
  ...
  std::transform(view.begin(), view.end(), std::back_insert_iterator(s),
  ...
  for (auto&& inst : std::move(s)) {
    ...
    case instruction::insert:
      hm.insert(std::forward<std::string>(inst.label), std::forward<std::uint64_t>(inst.value));
      continue;
  ...
  return std::transform_reduce(std::execution::par_unseq, iota.begin(), iota.end(),
    std::uint64_t{0}, std::plus<std::uint64_t>{}, [&buckets = hm.buckets](std::size_t bucket_id) {
