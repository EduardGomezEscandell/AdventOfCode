...
auto parse_and_expand_universe(std::string_view input, std::size_t expansion_rate) {
  ...
  // Expand columns
  std::for_each(std::execution::par_unseq, galaxies.begin(), galaxies.end(),
...
std::uint64_t Day11::solve(std::size_t expansion_rate) const {
  ...
  std::for_each(
    std::execution::par_unseq, iota.begin(), iota.end(), [&galaxies, &distances](std::size_t i) {
      auto& g1 = galaxies[i];
      for (std::size_t j = i + 1; j != galaxies.size(); ++j) {
        auto& g2 = galaxies[j];

        // Taxicab distance
        std::uint64_t vdistance = absolute_difference(g2.row, g1.row);
        std::uint64_t hdistance = absolute_difference(g2.col, g1.col);
  ...
  return std::reduce(std::execution::unseq, distances.begin(), distances.end(), std::uint64_t{0},
