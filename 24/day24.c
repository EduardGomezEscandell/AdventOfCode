...
#define NREGISTERS 4 // w x y z lit1 lit2
#define NARGUMENTS 2

address_t GetRegisterId(char repr)
{
    switch (repr) {
...
bool ReadArgument(char const * arg_begin, address_t * adress, value_t * value)
{
    ...
    {
        *adress = GetRegisterId(*arg_begin);
        return false;
    }
...
VirtualMachine ReadInput(const bool is_test)
{
    ...
    vm.instruction_ptr = REGISTER_SEGMENT_BEGIN;
...
{
    VirtualMachine vm = ReadInput(is_test);
    vm.data[IO_SEGMENT_BEGIN] = 5;

    PrintVirtualMachine(&vm);
